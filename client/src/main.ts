...
const handleSearchFormSubmit = (event: any): void => {
  ...
  if (!searchInput.value.trim()) { 
    alert('Please enter a city name'); // ✅ Show an alert instead
    return;
  }
...
// // const fetchSearchHistory = async (): Promise<HistoryItem[]> => {
// //   try {
...
// //   if (!heading || !weatherIcon || !tempEl || !windEl || !humidityEl || !todayContainer) return;
...
// //     ? historyList.map(buildHistoryListItem).join('')
// //     : '<p class="text-center">No Previous Search History</p>';
// // };
...
// // const buildHistoryListItem = (city: HistoryItem) => {
// //   return `<div class="display-flex gap-2 col-12 m-1">
// //     <button type="button" class="history-btn btn btn-secondary col-10" data-city="${city.name}">${city.name}</button>
// //     <button type="button" class="fas fa-trash-alt delete-city btn btn-danger col-2" data-id="${city.id}"></button>
// //   </div>`;
// // };
...
// //   } else if (target.matches('.delete-city')) {
// //     deleteCityFromHistory(target.dataset.id!);
// //   }
// // };
...
// // window.onload = getAndRenderHistory;
// // searchForm?.addEventListener('submit', handleSearchFormSubmit);
// // searchHistoryContainer?.addEventListener('click', handleSearchHistoryClick);
